...
<nav class="w3-sidebar w3-collapse w3-white w3-animate-left" style="z-index:3;width:300px;" id="mySidebar"><br>
  ...
  <div class="w3-bar-block">
    ...
    <a href="index" class="w3-bar-item w3-button w3-padding"><i class="fa fa-square"></i> Categorias</a>
...
<div class="w3-container"> <!--agregar -->
  <h5>
    <a href="{{ route('categorias.crear') }}" class="w3-button w3-black">Agregar</a>
  </h5>
  ...
  <table class="w3-table-all w3-text-theme w3-centered w3-theme-d1">
    <thead>
      ...
      {{-- hay que imprimir la consulta --}}
      {{-- tomamos nuestra variale categorias y la recorremos o iteramos --}}
      {{-- esta "->" se llama apuntador y sirve para accesar a las propiedades del objeto Category --}}
      {{-- ?? [] con esto le decimos que la variable podria venir vacia --}}
      @foreach($categorias ?? [] as $categoria)
      <tr>
        <td>{{$categoria->nombre}}</td>
        <td>{{$categoria->descripcion}}</td>
        <td>
          {{-- este debe ser un link  o deberia para que sea mas facil --}}
          <a class="w3-button w3-orange" href="{{ route('categorias.editar', $categoria) }}">&nbsp;&nbsp;Editar&nbsp;&nbsp;</a>
          <form method="post" action="{{route('categorias.eliminar',$categoria)}}">
            {{csrf_field()}}
            {{method_field('delete')}}
